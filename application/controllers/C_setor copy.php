    ...
    public function setorSampah($kode_transaksi)
{
    ...
    $this->session->set_flashdata('alert_1', '<div class="alert alert-success alert-dismissible">Item Sudah Berhasil Di tambahkan <i class="fa fa-check"></i></div>');
    redirect(base_url().'c_setor/v_insert/'.$kode_transaksi);
...
public function delete_item($kode_transaksi){
 ...
 public function edit_status($kode_transaksi,$nasabah_id){
    ...
    $nasabah = array('nasabah_id' => $nasabah_id);
        ...
        if($status == 'A_Aktif'){
            ...
            if (($sisa_saldo - $nominal_saldo) < 0){
                $this->session->set_flashdata('alert_2', '<div class="alert alert-danger">Jumlah Saldo = '.$sisa_saldo.', Status Transaksi Gagal diubah!</div>');
                redirect(base_url().'c_setor/index');
            }else{
            $data = array(
                'status' => $status
    ...
    $this->session->set_flashdata('alert_2', '<div class="alert alert-success">Status Transaksi Berhasil Diubah <i class="fa fa-check"></i></div>');
    redirect(base_url().'c_setor/index');
 }
}